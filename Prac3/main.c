...
int main(void)
{
 ...
 /*do while to make sure that if adc_val is 0, loop is executed once*/
  do{
      ...
      lcd_value[count] = (adc_val%10-0) + 48;
      adc_val=adc_val/10;
      count++;
  }while(adc_val);
...
void EXTI0_1_IRQHandler(void)
{
    ...
    currticks = HAL_GetTick(); /*get current tick count*/
    if((currticks-prevticks)>=debounceLength){
          if(prev_millis==0){
          /*Initial condition*/
          prev_millis = 1000;
          curr_millis = delay_t;
    ...
    prevticks = currticks; /*update previous tickcount*/
...
void writeLCD(char *char_in){
    ...
    while(count){
      count--;   
      lcd_putchar(char_in[count]); /*send character to LCD*/
...
uint32_t pollADC(void){
  ...
  uint32_t val = HAL_ADC_GetValue(&hadc);                     /*read current value*/
...
uint32_t ADCtoCCR(uint32_t adc_val){
  ...
  uint32_t val = ((adc_val*47999)/4095); /*max ADC value -> 100% Duty Cycle*/
